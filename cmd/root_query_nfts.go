...
func getUsersNfts(ctx context.Context, clientCtx client.Context, chain chains.Chain, address string) chains.NFTClass {
	...
	case chains.CosmWasm:
		classID := askForString("Class ID (full IBC path/trace or contract address)")
		...
		if len(splitClassID) <= 2 {
			log.Println("Expecting Class ID to be a contract address...")
			nftContract = classID
		} else {
		...
		nfts := queryOwnedNFTSFromContract(ctx, clientCtx, address, nftContract)
...
func queryOwnedNFTSFromContract(ctx context.Context, clientCtx client.Context, address string, nftContract string) []chains.NFT {
	wasmQueryClient := wasmdtypes.NewQueryClient(clientCtx)
